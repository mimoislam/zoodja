...
class SearchRepository{
  ...
  chooseUser(currentUserId,selectedUserId,name,photoUrl)async{
    await _firestore.collection("users").doc(currentUserId).collection("chosenList").doc(selectedUserId).set(
        {

        });
    await _firestore.collection("users").doc(selectedUserId).collection("chosenList").doc(currentUserId).set(
        {
          
        });
    
    await _firestore.collection("users").doc(selectedUserId).collection("selectedList").doc(currentUserId).set({
      "name":name,
      "photoUrl":photoUrl
    });
  ...
  getChosenList(userId)async{
    List<String>chosenList=[];
    await _firestore.collection("users").doc(userId).collection("chosenList").get().then((docs)  {
  ...
  Future<User>getUser(userId) async{
    ...
    await _firestore.collection("users").get().then((users){
      for (var user in users.docs ){
        ...
        if((!chosenList.contains(user.id))&&(user.id!=userId)&&(currentUser.interestedIn==user["gender"])&&
